...
function extractMarvelIssueFromGoogleHtml(html=''){
  ...
  for(const link of links){
    try{const p=new URL(link.replace(/&amp;/g,'&'),GOOGLE_ORIGIN),target=p.searchParams.get('q')||p.searchParams.get('url')||'';if(isMarvelIssueUrl(target))return cleanMarvelIssueUrl(target)}catch{}
  }
  return '';
}
...
async function resolveExactIssue(title,issue,year){
  ...
  for(const relaxed of [false,true]){
    try{
      ...
      const from=extractMarvelIssueFromGoogleHtml(await response.text());
      if(from)return from;
    }catch(e){console.error('Google normal resolver:',e)}
  }
  return '';
}
function absoluteImage(url=''){
  let v=unescapeHtml(url).trim();if(v.startsWith('//'))v='https:'+v;return /^https?:\/\//i.test(v)?v:'';
}
...
function buildSmartLink(drn,sourceId){
  ...
  const u=new URL(MARVEL_SMART_LINK);u.searchParams.set('type','issue');u.searchParams.set('drn',drn);u.searchParams.set('sourceId',sourceId);return u.toString();
}
function cacheKey(title,issue,year){
  const u=new URL('https://marvel-meta-cache.invalid/v6');u.searchParams.set('title',title);u.searchParams.set('issue',issue);u.searchParams.set('year',year);return new Request(u.toString());
}
async function readCache(key){
  const cache=typeof caches!=='undefined'?caches.default:null;if(!cache)return null;const hit=await cache.match(key);if(!hit)return null;try{return await hit.json()}catch{return null}
}
async function writeCache(key,data){
  const cache=typeof caches!=='undefined'?caches.default:null;if(!cache)return;await cache.put(key,Response.json(data,{headers:{'Cache-Control':`public, max-age=${META_TTL}`}})).catch(()=>{});
}
async function resolveUnifiedMeta(title,issue,year){
  const key=cacheKey(title,issue,year),cached=await readCache(key);if(cached?.smartLink)return cached;
  ...
  const data={resolverVersion:RESOLVER_VERSION,available:Boolean(smartLink),issueUrl,sourceId,readerId:reader.readerId,drn,smartLink,webUrl:reader.webUrl,coverUrl,pageTitle,reason:smartLink?'ok':reader.readerId?'drn-unavailable':'reader-unavailable'};
...
function knownMetaFromUrl(url){
  ...
  return{resolverVersion:RESOLVER_VERSION,available:true,issueUrl,sourceId:url.searchParams.get('knownSourceId')||sourceIdFromIssueUrl(issueUrl),readerId:url.searchParams.get('knownReaderId')||'',drn:url.searchParams.get('knownDrn')||'',smartLink,webUrl:url.searchParams.get('knownWebUrl')||issueUrl,coverUrl:'',pageTitle:url.searchParams.get('knownPageTitle')||'',reason:'client-cache'};
}

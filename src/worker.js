...
function isMarvelIssueUrl(value=''){try{const u=new URL(value,GOOGLE_ORIGIN);return /(^|\.)marvel\.com$/i.test(u.hostname)&&/^\/comics\/issue\/\d+\//i.test(u.pathname)}catch{return false}}
function cleanMarvelIssueUrl(value=''){try{const u=new URL(value,GOOGLE_ORIGIN);return `${u.protocol}//${u.host}${u.pathname}`}catch{return ''}}

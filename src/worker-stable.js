...
function significantTitleTokens(value=''){
  return normalizeText(value).split(/\s+/).filter(t=>t.length>1&&!STOP_WORDS.has(t));
}
...
function relaxedGoogleQuery(title,issue,year){
  return `site:marvel.com/comics/issue/ "${title}" "${issue?`#${issue}`:''}" ${year}`;
}
...
function normalGoogleUrl(title,issue,year,relaxed=false){
  return `${GOOGLE_ORIGIN}/search?q=${encodeURIComponent(relaxed?relaxedGoogleQuery(title,issue,year):exactGoogleQuery(title,issue,year))}`;
}
function isMarvelIssueUrl(value=''){
  try{
    const u=new URL(value,MARVEL_ORIGIN);
    return /(^|\.)marvel\.com$/i.test(u.hostname)&&/^\/comics\/issue\/\d+(?:\/|$)/i.test(u.pathname);
  }catch{return false}
}
function cleanMarvelIssueUrl(value=''){
  try{
    const u=new URL(value,MARVEL_ORIGIN);
    if(!isMarvelIssueUrl(u.href))return '';
    const m=u.pathname.match(/^\/comics\/issue\/\d+(?:\/[^?#]*)?/i);
    return `${u.protocol}//${u.host}${m?.[0]||u.pathname}`;
  }catch{return ''}
}
function sourceIdFromIssueUrl(issueUrl=''){
  try{return new URL(issueUrl).pathname.match(/^\/comics\/issue\/(\d+)/i)?.[1]||''}catch{return ''}
}
...
function extractMarvelIssueFromGoogleHtml(html=''){
  ...
  for(const link of links){
    try{
      const p=new URL(link.replace(/&amp;/g,'&'),GOOGLE_ORIGIN);
      const target=p.searchParams.get('q')||p.searchParams.get('url')||'';
      if(isMarvelIssueUrl(target))return cleanMarvelIssueUrl(target);
    }catch{}
  }
  return '';
...
function extractMarvelIssueCandidates(html=''){
  ...
  for(const raw of found){
    const u=cleanMarvelIssueUrl(raw.startsWith('http')?raw:MARVEL_ORIGIN+raw);
...
function extractReaderData(html,issueUrl){
  ...
  return{readerId:match?.[1]||'',webUrl:match?.[0]||issueUrl};
}
function absoluteImage(url=''){
  let v=unescapeHtml(url).trim();
...
function extractPageTitle(html=''){
  ...
  for(const p of patterns){const m=clean.match(p);if(m)return m[1].replace(/\s+/g,' ').trim()}
  return '';
}
function evaluateMatch(title,issue,year,pageTitle,issueUrl){
  const expectedTokens=significantTitleTokens(title);
  const actualTokens=significantTitleTokens(String(pageTitle).replace(/\|\s*Comic Issues.*$/i,'').replace(/\(Variant\)\s*$/i,''));
  ...
  let issueOk=true;
  if(rawIssue){
    const re=new RegExp(`#\\s*${escapeRegExp(rawIssue)}(?:\\b|\\s|$)`,'i');
    let slug='';try{slug=decodeURIComponent(new URL(issueUrl).pathname).toLowerCase()}catch{}
    const slugRe=new RegExp(`(?:_|-)${escapeRegExp(rawIssue)}(?:$|[_-])`,'i');
    issueOk=re.test(rawTitle)||slugRe.test(slug);
  }
  let yearOk=true;
  if(year){
    const y=String(year);
    yearOk=new RegExp(`\\(${escapeRegExp(y)}\\)`).test(rawTitle)||String(issueUrl).includes(`_${y}_`)||String(issueUrl).includes(`-${y}-`);
  }
...
// RESOLVER ESTABLE DE APERTURA.
// Este es el flujo que ya se comprobó funcionando en iOS: la identificación
// del número se hace con la búsqueda Google restringida y NO se somete al
// scoring del diagnóstico. No modificar por cambios en el diagnóstico.
async function resolveExactIssueForApp(title,issue,year){
  const lucky=luckyUrl(title,issue,year);
  try{
    const response=await fetchGoogle(lucky,'manual');
    const loc=unwrapGoogleLocation(response.headers.get('Location')||'');
    if(loc)return loc;
    if(isMarvelIssueUrl(response.url))return cleanMarvelIssueUrl(response.url);
    const html=await response.text(),from=extractMarvelIssueFromGoogleHtml(html);
    if(from)return from;
  }catch(e){console.error('Stable Google lucky resolver:',e)}
  for(const relaxed of [false,true]){
    try{
      const response=await fetchGoogle(normalGoogleUrl(title,issue,year,relaxed),'follow');
      const html=await response.text(),from=extractMarvelIssueFromGoogleHtml(html);
      if(from)return from;
    }catch(e){console.error('Stable Google normal resolver:',e)}
  }
  return '';
}

// RESOLVER DE DESCUBRIMIENTO.
// Puede evolucionar para precarga/diagnóstico sin afectar al botón de apertura.
async function resolveExactIssueForDiscovery(title,issue,year){
  ...
  for(const query of queries){
    try{
      ...
      for(const candidate of candidates){
        ...
        try{
          const issueHtml=await fetchHtml(candidate);
          const pageTitle=extractPageTitle(issueHtml),match=evaluateMatch(title,issue,year,pageTitle,candidate);
          if(!match.possibleMismatch)return{issueUrl:candidate,resolverSource:'marvel-search'};
        }catch(e){console.error('Marvel candidate:',e)}
      }
    }catch(e){console.error('Marvel search resolver:',e)}
  }
  const google=await resolveExactIssueForApp(title,issue,year);
  if(google)return{issueUrl:google,resolverSource:'google-fallback'};
  return{issueUrl:'',resolverSource:'unresolved'};
}
async function resolveLegacyDrn(readerId){
  if(!readerId)return '';
  const html=await fetchHtml(`${MARVEL_LEGACY_SHARE}${encodeURIComponent(readerId)}`),clean=unescapeHtml(html).replace(/%3A/gi,':');
  let explicit=clean.match(/(?:[?&]|\b)drn=([^&"'<>\s]+)/i)?.[1]||'';
...
function buildSmartLink(drn,sourceId){
  ...
  const u=new URL(MARVEL_SMART_LINK);
  u.searchParams.set('type','issue');u.searchParams.set('drn',drn);u.searchParams.set('sourceId',sourceId);
  return u.toString();
}
function cacheKey(kind,title,issue,year){
  const u=new URL('https://marvel-meta-cache.invalid/item');
  u.searchParams.set('resolver',String(RESOLVER_VERSION));u.searchParams.set('kind',kind);u.searchParams.set('title',title);u.searchParams.set('issue',issue);u.searchParams.set('year',year);
  return new Request(u.toString());
}
async function metadataFromIssueUrl(issueUrl,title,issue,year,resolverSource,{strict=false}={}){
  const sourceId=sourceIdFromIssueUrl(issueUrl),html=await fetchHtml(issueUrl);
  const {readerId,webUrl}=extractReaderData(html,issueUrl),coverUrl=extractCoverUrl(html),pageTitle=extractPageTitle(html);
  const match=evaluateMatch(title,issue,year,pageTitle,issueUrl);
  if(strict&&match.possibleMismatch){
    return{resolverVersion:RESOLVER_VERSION,resolverSource,available:false,issueUrl,sourceId,readerId:'',drn:'',smartLink:'',webUrl:issueUrl,coverUrl,pageTitle,reason:'possible-mismatch',match};
  }
  let drn='',smartLink='';
  if(readerId&&sourceId){
    try{drn=await resolveLegacyDrn(readerId);smartLink=buildSmartLink(drn,sourceId)}catch(e){console.error('Legacy DRN:',e)}
  }
  return{resolverVersion:RESOLVER_VERSION,resolverSource,available:Boolean(smartLink),issueUrl,sourceId,readerId,drn,smartLink,webUrl,coverUrl,pageTitle,reason:smartLink?'ok':readerId?'drn-unavailable':'reader-unavailable',match};
}
...
async function writeCache(key,data){
  ...
  const response=Response.json(data,{headers:{'Cache-Control':`public, max-age=${META_TTL}`}});
  await cache.put(key,response.clone()).catch(()=>{});
}
async function resolveAppMeta(title,issue,year){
  const key=cacheKey('app-stable',title,issue,year),cached=await readCache(key);if(cached)return cached;
  let issueUrl=await resolveExactIssueForApp(title,issue,year),resolverSource='google-stable';
  if(!issueUrl){
    const fallback=await resolveExactIssueForDiscovery(title,issue,year);issueUrl=fallback.issueUrl;resolverSource=fallback.resolverSource;
  }
  if(!issueUrl)return{resolverVersion:RESOLVER_VERSION,resolverSource:'unresolved',available:false,issueUrl:'',sourceId:'',readerId:'',drn:'',smartLink:'',webUrl:luckyUrl(title,issue,year),coverUrl:'',pageTitle:'',reason:'lookup-unresolved'};
  // Importante: el flujo interactivo no rechaza el resultado por scoring.
  // La identificación Google restringida fue la que se validó manualmente.
  const data=await metadataFromIssueUrl(issueUrl,title,issue,year,resolverSource,{strict:false});
  if(data.smartLink)await writeCache(key,data);
  return data;
}
async function resolveDiscoveryMeta(title,issue,year){
  const key=cacheKey('discovery',title,issue,year),cached=await readCache(key);if(cached)return cached;
  const resolved=await resolveExactIssueForDiscovery(title,issue,year),issueUrl=resolved.issueUrl;
  ...
  const data=await metadataFromIssueUrl(issueUrl,title,issue,year,resolved.resolverSource,{strict:true});
  // Los negativos ambiguos no se cachean durante 30 días. Solo datos positivos
  // o ausencias de lector confirmadas en una ficha exacta.
  if(data.smartLink||data.reason==='reader-unavailable')await writeCache(key,data);
  return data;
}
async function verifyUrl(url){
  if(!url)return{ok:false,status:0,location:'',error:'missing-url'};
  try{
    const response=await fetch(url,{method:'GET',redirect:'manual',headers:{'User-Agent':'Mozilla/5.0 (compatible; MarvelLectura-Diagnostic/1.3)','Accept':'text/html,*/*;q=0.8'}});
    return{ok:response.status>=200&&response.status<400,status:response.status,location:response.headers.get('Location')||'',error:''};
  }catch(e){return{ok:false,status:0,location:'',error:String(e?.message||e)}}
}
function knownMetaFromUrl(url){
  ...
  return{
    resolverVersion:RESOLVER_VERSION,resolverSource:'client-cache',available:true,issueUrl,
    sourceId:url.searchParams.get('knownSourceId')||sourceIdFromIssueUrl(issueUrl),readerId:url.searchParams.get('knownReaderId')||'',
    drn:url.searchParams.get('knownDrn')||'',smartLink,webUrl:url.searchParams.get('knownWebUrl')||issueUrl,
    coverUrl:'',pageTitle:url.searchParams.get('knownPageTitle')||'',reason:'client-cache'
  };
}
async function diagnosticMeta(title,issue,year,known=null){
  const meta=known||await resolveDiscoveryMeta(title,issue,year);
...
function errorPage(fallback,msg='No he podido construir el enlace móvil de Marvel Unlimited.'){
  const safe=String(fallback).replace(/&/g,'&amp;').replace(/"/g,'&quot;');
  const text=String(msg).replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]));
...
export default{
  async fetch(request,env){
    ...
    try{
      if(mode==='diagnostic'){
        const data=await diagnosticMeta(title,issue,year,knownMetaFromUrl(url));
        return Response.json({title,issue,year,...data},{headers:{'Cache-Control':'no-store'}});
      }
      if(mode==='meta'){
        const meta=await resolveDiscoveryMeta(title,issue,year);
        return Response.json(meta,{headers:{'Cache-Control':'private, no-store'}});
      }
      if(mode==='debug'){
        const meta=await resolveAppMeta(title,issue,year);
        return Response.json({title,issue,year,...meta},{headers:{'Cache-Control':'no-store'}});
      }
      ...
      if(mode==='meta'||mode==='diagnostic')return Response.json({resolverVersion:RESOLVER_VERSION,available:false,webUrl:lucky,reason:'resolver-error',diagnosticCode:'RESOLVER_ERROR',error:String(e?.message||e)},{status:200,headers:{'Cache-Control':'no-store'}});

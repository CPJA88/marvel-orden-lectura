...
function bingUrl(title,issue,year){return `${BING_ORIGIN}/search?q=${encodeURIComponent(relaxedGoogleQuery(title,issue,year))}`}
...
function isMarvelIssueUrl(value=''){
  try{const u=new URL(value,MARVEL_ORIGIN);return /(^|\.)marvel\.com$/i.test(u.hostname)&&/^\/comics\/issue\/\d+(?:\/|$)/i.test(u.pathname)}catch{return false}
}
function cleanMarvelIssueUrl(value=''){
  try{const u=new URL(value,MARVEL_ORIGIN);if(!isMarvelIssueUrl(u.href))return '';const m=u.pathname.match(/^\/comics\/issue\/\d+(?:\/[^?#]*)?/i);return `${u.protocol}//${u.host}${m?.[0]||u.pathname}`}catch{return ''}
}
...
function extractMarvelIssueCandidates(html=''){
  ...
  for(const raw of found){
    const candidate=cleanMarvelIssueUrl(raw.startsWith('http')?raw:MARVEL_ORIGIN+raw);
    if(candidate&&!seen.has(candidate)){seen.add(candidate);out.push(candidate)}
  }
  return out;
}
function extractMarvelIssueFromGoogleHtml(html=''){return extractMarvelIssueCandidates(html)[0]||''}
async function fetchSearch(url,redirect='follow'){
...
function extractPageTitle(html=''){
  ...
  for(const p of [/<meta[^>]+property=["']og:title["'][^>]+content=["']([^"']+)["']/i,/<meta[^>]+content=["']([^"']+)["'][^>]+property=["']og:title["']/i,/<title[^>]*>([^<]+)<\/title>/i]){
    const m=clean.match(p);if(m)return m[1].replace(/\s+/g,' ').trim();
  }
  return '';
}
function candidateMatches(title,issue,year,pageTitle,issueUrl){
  const expected=normalizeText(title),actual=normalizeText(String(pageTitle).replace(/\|\s*Comic Issues.*$/i,''));
  const titleOk=expected?actual.includes(expected):true;
  const rawIssue=String(issue||'').trim();
  const issueOk=!rawIssue||new RegExp(`#\\s*${escapeRegExp(rawIssue)}(?:\\b|\\s|$)`,'i').test(pageTitle)||new RegExp(`(?:_|-)${escapeRegExp(rawIssue)}(?:$|[_-])`,'i').test(String(issueUrl));
  const rawYear=String(year||'').trim();
  const yearOk=!rawYear||new RegExp(`\\(${escapeRegExp(rawYear)}\\)`).test(pageTitle)||String(issueUrl).includes(`_${rawYear}_`)||String(issueUrl).includes(`-${rawYear}-`);
  return titleOk&&issueOk&&yearOk;
}
async function chooseExactCandidate(candidates,title,issue,year){
  for(const candidate of candidates.slice(0,12)){
    try{
      const html=await fetchHtml(candidate),pageTitle=extractPageTitle(html);
      if(candidateMatches(title,issue,year,pageTitle,candidate))return candidate;
    }catch(e){console.error('Candidate validation:',e)}
  }
  return '';
}
async function resolveExactIssue(title,issue,year){
  ...
  try{
    const response=await fetchSearch(lucky,'manual'),loc=unwrapGoogleLocation(response.headers.get('Location')||'');
    if(loc)return{issueUrl:loc,resolverSource:'google-lucky'};
    if(isMarvelIssueUrl(response.url))return{issueUrl:cleanMarvelIssueUrl(response.url),resolverSource:'google-lucky'};
    const from=extractMarvelIssueFromGoogleHtml(await response.text());if(from)return{issueUrl:from,resolverSource:'google-lucky-html'};
  }catch(e){console.error('Google lucky resolver:',e)}

  try{
    const response=await fetchSearch(normalGoogleUrl(title,issue,year,false),'follow');
    const from=extractMarvelIssueFromGoogleHtml(await response.text());
    if(from)return{issueUrl:from,resolverSource:'google-exact'};
  }catch(e){console.error('Google exact resolver:',e)}

  try{
    const response=await fetchSearch(marvelSearchUrl(title,issue,year),'follow');
    if(response.ok){
      const candidate=await chooseExactCandidate(extractMarvelIssueCandidates(await response.text()),title,issue,year);
      if(candidate)return{issueUrl:candidate,resolverSource:'marvel-search'};
    }
  }catch(e){console.error('Marvel search resolver:',e)}

  try{
    const response=await fetchSearch(bingUrl(title,issue,year),'follow');
    if(response.ok){
      const candidate=await chooseExactCandidate(extractMarvelIssueCandidates(await response.text()),title,issue,year);
      if(candidate)return{issueUrl:candidate,resolverSource:'bing-fallback'};
    }
  }catch(e){console.error('Bing resolver:',e)}

  try{
    const response=await fetchSearch(normalGoogleUrl(title,issue,year,true),'follow');
    if(response.ok){
      const candidate=await chooseExactCandidate(extractMarvelIssueCandidates(await response.text()),title,issue,year);
      if(candidate)return{issueUrl:candidate,resolverSource:'google-relaxed'};
    }
  }catch(e){console.error('Google relaxed resolver:',e)}
...
function currentCacheKey(title,issue,year){const u=new URL('https://marvel-meta-cache.invalid/v7');u.searchParams.set('title',title);u.searchParams.set('issue',issue);u.searchParams.set('year',year);return new Request(u.toString())}
function legacyCacheKeys(title,issue,year){
  const keys=[];
  {const u=new URL('https://marvel-meta-cache.invalid/v6');u.searchParams.set('title',title);u.searchParams.set('issue',issue);u.searchParams.set('year',year);keys.push(new Request(u.toString()))}
...
async function diagnosticMeta(title,issue,year,known=null){
  ...
  else if(!meta.readerId&&meta.reason==='reader-unavailable')diagnosticCode='NOT_IN_UNLIMITED';

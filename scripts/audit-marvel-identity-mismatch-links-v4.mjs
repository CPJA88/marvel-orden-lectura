...
async function inspect(row,meta){
  const r=await get(ISSUE+row.sourceId);
  if(r.status===404||r.status===410)return{gcdId:row.gcdId,sourceId:row.sourceId,kind:'not-found'};
  const html=await r.text(),rid=readerId(html),d=drn(html),c=cover(html),av=availability(html);
  if(av!=='mu')return{gcdId:row.gcdId,sourceId:row.sourceId,kind:'availability-regression',availability:av};

  let reader={ok:false,reason:'missing-reader-id'};
  if(rid){
    try{
      const rr=await get(SHARE+rid,{tries:3});
      if(rr.ok){
        const t=title(await rr.text()),p=parse(t);
        reader={
          ok:Boolean(p&&issue(p.issue)===issue(meta.localIssue)&&series(p.series)===series(meta.actualSeries)),
          title:t,
          smartLink:`${SMART}?type=reader&drn=${rid}`
        };
      }
    }catch(e){
      reader={ok:false,transient:true,reason:e?.message||String(e)};
    }
  }

  let uuid={ok:false,reason:'missing-drn'};
  if(d){
    try{
      const ur=await get(`${SMART}?type=issue&drn=${encodeURIComponent(d)}&sourceId=${row.sourceId}`,{manual:true,tries:2});
      uuid={ok:ur.status>=200&&ur.status<400&&![403,429].includes(ur.status),status:ur.status,location:ur.headers.get('location')||''};
    }catch(e){
      uuid={ok:false,transient:true,reason:e?.message||String(e)};
    }
  }

  return{gcdId:row.gcdId,sourceId:row.sourceId,readerId:rid,drn:d,cover:c,kind:'mu',reader,uuid,functional:Boolean(reader.ok||uuid.ok)};
}
...
for(const row of pilot.results){
  const cur=byId.get(Number(row.gcdId));
  if(!cur||Number(cur[3])!==4)throw new Error(`GCD ${row.gcdId} dejó de ser NOT_LISTED.`);
  if(terminalSource.has(Number(row.sourceId)))throw new Error(`sourceId ocupado: ${row.sourceId}`);
  const m=meta.get(Number(row.gcdId));
  if(!m)throw new Error(`Sin metadata ${row.gcdId}`);
  try{results.push(await inspect(row,m))}catch(e){results.push({gcdId:row.gcdId,sourceId:row.sourceId,kind:'retryable',reason:e?.message||String(e)})}
  if(results.length%25===0)console.log(`${results.length}/429`);
  await sleep(120);
}

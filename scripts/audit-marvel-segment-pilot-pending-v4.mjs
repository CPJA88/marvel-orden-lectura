...
function addOriginVariants(urls,origin,sourceId,segment,row){const base=`${origin}${sourceId}`;urls.push(base,`${base}/read`,`${base}/ajax/aggregate`,`${base}?mobile-app=true`);for(const slug of issueSlugVariants(segment,row))urls.push(`${base}/${slug}`,`${base}/${slug}/read`,`${base}/${slug}/ajax/aggregate`,`${base}/${slug}?mobile-app=true`)}
function issueUrlVariants(sourceId,segment,row){const urls=[];addOriginVariants(urls,SHARE_ISSUE,sourceId,segment,row);addOriginVariants(urls,ISSUE,sourceId,segment,row);return[...new Set(urls)]}
async function getIssue(sourceId,segment,row){const attempts=[];for(const url of issueUrlVariants(sourceId,segment,row)){try{const r=await get(url,2);attempts.push({url,status:r.status,finalUrl:r.url||url});if(r.ok)return{response:r,attempts}}catch(e){attempts.push({url,status:0,error:e?.message||String(e)})}await sleep(35)}return{response:null,attempts}}
...
async function inspect(segment,row){
  ...
  const fetched=await getIssue(sourceId,segment,row),r=fetched.response;if(!r)return{kind:'not-found',sourceId,attempts:fetched.attempts};
  const issueUrl=r.url||'',html=await r.text(),ts=titles(html),id=identity(ts,segment,row,{requireVolumeYear:true});if(!id.ok)return{kind:'identity-fail',sourceId,issueUrl,attempts:fetched.attempts,titles:id.titles};
  const officialDate=pageDate(html);if(!officialDate||officialDate!==expectedDate)return{kind:'date-fail',sourceId,issueUrl,attempts:fetched.attempts,expectedDate,officialDate,officialTitle:id.title};
  const av=availability(html),rid=readerId(html);
  if(av==='no-digital')return{kind:'no-digital',sourceId,issueUrl,attempts:fetched.attempts,readerId:rid,officialDate,officialTitle:id.title};
  if(av!=='mu')return{kind:'unknown',sourceId,issueUrl,attempts:fetched.attempts,availability:av,readerId:rid,officialDate,officialTitle:id.title};
  if(!expectedReaderId||rid!==expectedReaderId)return{kind:'reader-id-fail',sourceId,issueUrl,attempts:fetched.attempts,expectedReaderId,readerId:rid,officialDate,officialTitle:id.title};
  const rr=await get(SHARE+rid);if(!rr.ok)return{kind:'reader-http-fail',sourceId,issueUrl,attempts:fetched.attempts,readerId:rid,http:rr.status,officialDate,officialTitle:id.title};
  const rh=await rr.text(),rt=titles(rh),ri=identity(rt,segment,row,{requireVolumeYear:false});if(!ri.ok)return{kind:'reader-identity-fail',sourceId,issueUrl,attempts:fetched.attempts,readerId:rid,titles:ri.titles,officialDate,officialTitle:id.title};
  return{kind:'mu',sourceId,issueUrl,attempts:fetched.attempts,readerId:rid,officialDate,officialTitle:id.title,readerTitle:ri.title,functional:true};
}

...
function issueUrlVariants(sourceId,segment,row){const base=`${ISSUE}${sourceId}`,urls=[base,`${base}/read`,`${base}/ajax/aggregate`,`${base}?mobile-app=true`];for(const slug of issueSlugVariants(segment,row)){urls.push(`${base}/${slug}`,`${base}/${slug}/read`,`${base}/${slug}/ajax/aggregate`,`${base}/${slug}?mobile-app=true`)}return[...new Set(urls)]}
async function getIssue(sourceId,segment,row){const attempts=[];for(const url of issueUrlVariants(sourceId,segment,row)){try{const r=await get(url,2);attempts.push({url,status:r.status,finalUrl:r.url||url});if(r.ok)return{response:r,attempts}}catch(e){attempts.push({url,status:0,error:e?.message||String(e)})}await sleep(45)}return{response:null,attempts}}

...
async function inspect(segment,row){
  ...
  const r=await get(ISSUE+sourceId);if(r.status===404||r.status===410)return{kind:'not-found',sourceId,http:r.status};
  const html=await r.text(),ts=titles(html),id=identity(ts,segment,row,{requireVolumeYear:true});if(!id.ok)return{kind:'identity-fail',sourceId,titles:id.titles};
  const officialDate=pageDate(html);if(!officialDate||officialDate!==expectedDate)return{kind:'date-fail',sourceId,expectedDate,officialDate,officialTitle:id.title};
  const av=availability(html),rid=readerId(html);
  if(av==='no-digital')return{kind:'no-digital',sourceId,readerId:rid,officialDate,officialTitle:id.title};
  if(av!=='mu')return{kind:'unknown',sourceId,availability:av,readerId:rid,officialDate,officialTitle:id.title};
  if(!expectedReaderId||rid!==expectedReaderId)return{kind:'reader-id-fail',sourceId,expectedReaderId,readerId:rid,officialDate,officialTitle:id.title};
  const rr=await get(SHARE+rid);if(!rr.ok)return{kind:'reader-http-fail',sourceId,readerId:rid,http:rr.status,officialDate,officialTitle:id.title};
  const rh=await rr.text(),rt=titles(rh),ri=identity(rt,segment,row,{requireVolumeYear:false});if(!ri.ok)return{kind:'reader-identity-fail',sourceId,readerId:rid,titles:ri.titles,officialDate,officialTitle:id.title};
  return{kind:'mu',sourceId,readerId:rid,officialDate,officialTitle:id.title,readerTitle:ri.title,functional:true};
}

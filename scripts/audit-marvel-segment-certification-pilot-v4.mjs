...
function pageDate(html){const raw=str(html),text=plain(html);for(const re of [/["'](?:onSaleDate|onsaleDate|datePublished)["']\s*:\s*["'](\d{4}-\d{2}-\d{2})/i]){const m=raw.match(re);if(m)return m[1]}for(const re of [/\bPublished\s+([A-Z][a-z]+\s+\d{1,2},\s+\d{4})\b/i]){const m=text.match(re);if(m){const d=new Date(m[1]);if(!Number.isNaN(d.valueOf()))return d.toISOString().slice(0,10)}}return''}

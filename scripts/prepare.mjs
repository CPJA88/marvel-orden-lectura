...
const uiVersion='v1.2.10-trace-diagnostic';

for(const file of [archive,path.join(source,'index.html'),path.join(source,'styles.css'),path.join(source,'enhancements.css'),path.join(source,'diagnostics.css'),path.join(source,'app.js'),path.join(source,'diagnostics.js'),path.join(source,'resolver-ui.js'),path.join(source,'cache-ui.js'),path.join(source,'diagnostic-trace.js')]){
...
sw=sw.replace("'styles.css','app.js'","'styles.css','enhancements.css','diagnostics.css','app.js','diagnostics.js','resolver-ui.js','cache-ui.js','diagnostic-trace.js'");
...
for(const required of ['index.html','app.js','diagnostics.js','resolver-ui.js','cache-ui.js','diagnostic-trace.js','styles.css','enhancements.css','diagnostics.css','manifest.webmanifest','sw.js','data/meta.json','data/search.json','data/series.json'])await fs.access(path.join(output,required));
console.log(`PWA Marvel construida con ${uiVersion}: diagnóstico con traza HTTP detallada del resolver; apertura Smart Link y metadata UI sin cambios.`);

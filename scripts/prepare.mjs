...
const uiVersion='v1.2.19-marvel-api';

for(const file of [archive,path.join(source,'index.html'),path.join(source,'styles.css'),path.join(source,'enhancements.css'),path.join(source,'diagnostics.css'),path.join(source,'app.js'),path.join(source,'diagnostics.js'),path.join(source,'resolver-ui.js'),path.join(source,'cache-ui.js'),path.join(source,'diagnostic-trace.js'),path.join(source,'diagnostic-v9-fix.js')]){
...
await Promise.all([
  ...
  fs.copyFile(path.join(source,'diagnostic-v9-fix.js'),path.join(output,'diagnostic-v9-fix.js')),
]);
...
if(!index.includes('diagnostic-trace.js'))index=index.replace('<script src="cache-ui.js" defer></script>','<script src="cache-ui.js" defer></script>\n<script src="diagnostic-trace.js" defer></script>');
if(!index.includes('diagnostic-v9-fix.js'))index=index.replace('<script src="diagnostic-trace.js" defer></script>','<script src="diagnostic-trace.js" defer></script>\n<script src="diagnostic-v9-fix.js" defer></script>');
...
sw=sw.replace("'styles.css','app.js'","'styles.css','enhancements.css','diagnostics.css','app.js','diagnostics.js','resolver-ui.js','cache-ui.js','diagnostic-trace.js','diagnostic-v9-fix.js'");
...
for(const required of ['index.html','app.js','diagnostics.js','resolver-ui.js','cache-ui.js','diagnostic-trace.js','diagnostic-v9-fix.js','styles.css','enhancements.css','diagnostics.css','manifest.webmanifest','sw.js','data/meta.json','data/search.json','data/series.json'])await fs.access(path.join(output,required));
console.log(`PWA Marvel construida con ${uiVersion}: portadas por GCD y metadata Unlimited por Marvel Comics API oficial. El HTML público de marvel.com queda fuera del resolver de metadata.`);

...
const fallbackSource=await fs.readFile('source/marvel-reader-fallback-v1240.js','utf8');
const stabilitySource=await fs.readFile('source/stability-v22.js','utf8');
...
const context={state:{marvel},unlimitedState:m=>({label:Number(m?.preinstalledStatus)===5?'Unlimited ✓ · enlace pendiente':'BASE',cls:'base'}),stableAppHref:()=> 'BASE-HREF',marvelQuery:()=> 'QUERY-HREF',updateRenderedMeta:()=>{},requestAnimationFrame:fn=>fn(),setTimeout:fn=>fn(),fetch:async()=>({ok:true,json:async()=>({version:3,officiallyVerified:true})}),document:{querySelectorAll:()=>[]},window:{},console};
vm.createContext(context);
vm.runInContext(fallbackSource,context,{filename:'marvel-reader-fallback-v1240.js'});
vm.runInContext(stabilitySource,context,{filename:'stability-v22.js'});
...
console.log(`Fallback reader validado tras la capa de estabilidad: ${checked} objetivos verificados; ninguna entrada fuera de allowlist afectada.`);
